<?php

$pageTitle = "Products"; // This will be used in the title tag
$pageDescription = "Ciao Football's Product's Page. View our premium football products and browse around."; // This is used as the page desciption meta tag
$pageKeywords = "Ciao, Football, Ciao Football, Soccer, replica, shirt, football shirts, equipment, store, premium, products page, products"; // This is used as the keywords meta tag

// Include the header file
include('../components/header.php');
// Inlude the footer file
include('../components/footer.php');
?>

<!-- Main content of the page starts here -->
<main>
  <section class="hero">
    <h1>Products</h1>
    <p>Your destination for premium football gear</p>
  </section>
  
</main>



<script src="../js/app.js"></script>
<script src="https://unpkg.com/lucide@latest/dist/umd/lucide.js"></script>
</body>
</html>